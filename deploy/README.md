# نشر سيركل داخل السعودية (Oracle Cloud – جدة)

كل شيء يعمل على سيرفر واحد في منطقة جدة: الموقع ولوحات التحكم، وقاعدة البيانات
(Supabase مستضافة ذاتياً)، وn8n للأتمتة، وCaddy لشهادات HTTPS. بيانات المرضى لا تخرج من
المملكة، والنسخ الاحتياطية مشفّرة وتُحفظ في تخزين أوراكل في جدة أيضاً.
...
2. **Home Region: اختر `Saudi Arabia West (Jeddah)`** (أو الرياض إن كانت متاحة). لا يمكن تغييرها لاحقاً، والموارد
...
## الخطوة 5: مكان النسخ الاحتياطية (في جدة)
...
2. من صفحة الـ bucket انسخ **Namespace**. الـ Endpoint يكون:
   `https://<namespace>.compat.objectstorage.me-jeddah-1.oraclecloud.com`
